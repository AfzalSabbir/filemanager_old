...
<div class="col-12">
  <div class="row">
    ...
    <div class=" offset-lg-6 col-12 col-lg-6 pt-2 d-none d-lg-block">
      ...
      @else
         <label style="float: right">{{ $table->appends(['items' => $items])->onEachSide(2)->links('frontend.partials.paginations.book.bootstrap-4') }}</label>
      @endif
    ...
    <div class="col-12 col-lg-6 pt-2 d-block d-lg-none">
      @if(isset($where))
        <label style="float: right">{{ $table->appends(\Request::query())->render() }}</label>
      @else
         <label style="float: right">{{ $table->appends(['items' => $items])->onEachSide(1)->links('frontend.partials.paginations.book.bootstrap-4-sm') }}</label>
      @endif
